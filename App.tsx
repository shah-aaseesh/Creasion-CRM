...
const App: React.FC = () => {
  ...
  const saveService = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const serviceData: Service = {
      ...
      additionalServices: editItem?.additionalServices || [],
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans no-select relative">
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        <header className="bg-white h-20 border-b flex items-center justify-between px-4 md:px-10 shrink-0 z-10 shadow-sm">
          <div className="flex items-center space-x-3 md:space-x-8">
            ...
            <h2 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight">{activeTab === 'services' ? 'Inventory' : 'System'}</h2>
          ...
          <button onClick={() => { setEditItem(null); setFormType('both'); setIsModalOpen(true); }} className="bg-indigo-600 text-white p-2.5 md:px-8 md:py-3.5 rounded-xl md:rounded-2xl flex items-center space-x-3 hover:bg-indigo-700 shadow-xl shadow-indigo-100 font-black uppercase text-[10px] md:text-xs tracking-widest transition-all">
            ...
            <div className="space-y-6 md:space-y-10">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                 <div className="bg-white p-6 md:p-10 rounded-[2rem] border-2 border-slate-100 shadow-sm flex items-center justify-between">
                   <div><p className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-2">Total Assets</p><p className="text-3xl md:text-5xl font-black text-slate-900">{stats.total}</p></div>
                 ...
                 <div className="bg-white p-6 md:p-10 rounded-[2rem] border-2 border-rose-100 bg-rose-50/10 shadow-sm flex items-center justify-between">
                   <div><p className="text-[10px] font-black text-rose-600 uppercase tracking-widest mb-2">Urgent Alerts</p><p className="text-3xl md:text-5xl font-black text-rose-600">{stats.alerts.length}</p></div>
              ...
              <div className="bg-white rounded-2xl md:rounded-[2rem] border-2 border-slate-100 overflow-hidden shadow-sm">
                <div className="px-6 md:px-8 py-5 border-b-2 border-slate-50 bg-slate-50/50 flex items-center justify-between">
                  <h3 className="font-black text-slate-900 uppercase tracking-widest text-[10px] flex items-center gap-2"><Clock size={16}/> Upcoming Expiries</h3>
                  {isSyncing && <Loader2 className="animate-spin text-indigo-500" size={14} />}
                </div>
                <div className="divide-y-2 divide-slate-50">
                  {stats.alerts.length === 0 ? (
                    <div className="p-10 text-center text-slate-300 font-bold uppercase tracking-widest text-xs italic">All systems nominal</div>
                  ) : (
                ...
                <table className="w-full text-left min-w-[600px]">
                  <thead className="bg-slate-50/80 border-b-2 border-slate-100">
                    <tr>
                      ...
                      <th className="px-6 py-6 text-[10px] font-black text-slate-600 uppercase tracking-widest">Asset Name</th>
                      ...
                      return (
                        <React.Fragment key={service.id}>
                          <tr className={`hover:bg-slate-50 transition-all cursor-pointer ${isExpanded ? 'bg-indigo-50/30' : ''}`} onClick={() => toggleRow(service.id)}>
                            ...
                            <td className="px-6 py-6"><p className="font-black text-slate-900 tracking-tighter text-lg">{service.domainName || service.hostingPlan || 'Managed Service'}</p></td>
                            <td className="px-6 py-6"><div className="flex gap-2"><span className={`text-[9px] font-black uppercase px-2 py-1 rounded-lg border ${service.type === 'domain' ? 'bg-indigo-50 text-indigo-700 border-indigo-100' : 'bg-orange-50 text-orange-700 border-orange-100'}`}>{service.type}</span></div></td>
                            ...
                            <td className="px-6 py-6 text-right" onClick={e => e.stopPropagation()}>
                              <div className="flex justify-end space-x-2">
                                <button onClick={() => { setEditItem(service); setFormType(service.type); setIsModalOpen(true); }} className="p-2 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl"><FileText size={18}/></button>
                                <button onClick={() => deleteItem(service.id)} className="p-2 text-slate-500 hover:text-rose-600 hover:bg-rose-50 rounded-xl"><Trash2 size={18}/></button>
                              </div>
                            </td>
                          </tr>
                          {isExpanded && (
                                ...
                                <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
                                    <div className="space-y-6">
                                      ...
                                      <div className="bg-white p-6 rounded-3xl border-2 border-slate-100 space-y-4 shadow-sm relative overflow-hidden">
                                          {(service.type === 'domain' || service.type === 'both') && (
                                            <div className="space-y-4"><p className="text-lg font-black text-slate-900 tracking-tighter">{service.domainName} <span className="text-[10px] text-slate-400 font-bold">({service.registrar})</span></p>
                                          ...
                                          {(service.type === 'hosting' || service.type === 'both') && (
                                            <div className="space-y-4"><p className="text-lg font-black text-slate-900 tracking-tighter">{service.hostingPlan} <span className="text-[10px] text-slate-400 font-bold">({service.hostingProvider})</span></p>
                                    ...
                                    <div className="space-y-6">
                                      ...
                                      <div className="space-y-4">
                                        ...
                                        )) : <p className="text-xs text-slate-400 italic font-medium">No additional services linked.</p>}
                                      </div>
                                    </div>
                                    <div className="space-y-6">
                                      ...
                                      <div className="bg-slate-900 text-white p-8 rounded-[2.5rem] shadow-xl min-h-[200px] relative overflow-hidden">
                                          <p className="relative z-10 text-sm font-medium leading-relaxed opacity-90 italic whitespace-pre-wrap">{service.notes || 'No entries.'}</p>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editItem ? 'Edit Asset' : 'New Provision'}>
         <form onSubmit={saveService} className="space-y-8 pb-10">
            ...
            {(formType === 'domain' || formType === 'both') && (
              <div className="p-8 rounded-[2.5rem] border-2 border-indigo-200 bg-white space-y-6 shadow-sm">
                <p className="text-[11px] font-black text-indigo-900 uppercase tracking-widest flex items-center gap-2"><Globe size={18}/> Domain Matrix</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="md:col-span-2"><input name="domainName" defaultValue={editItem?.domainName} className="w-full px-5 py-3.5 border-2 border-slate-300 rounded-2xl focus:border-indigo-600 outline-none font-black text-indigo-900" placeholder="example.com" /></div>
                  <input name="registrar" defaultValue={editItem?.registrar} className="w-full px-5 py-3 border-2 border-slate-300 rounded-2xl text-xs font-bold" placeholder="Registrar" />
                  ...
                  <div className="md:col-span-2 grid grid-cols-2 gap-4">
                     <div className="flex border-2 border-rose-200 rounded-2xl overflow-hidden"><select name="domainCostCurrency" defaultValue={editItem?.domainCostCurrency || 'NPR'} className="bg-rose-50 px-2 outline-none font-black text-[9px]"><option value="NPR">NPR</option><option value="INR">INR</option></select><input name="domainCost" type="number" step="0.01" defaultValue={editItem?.domainCost} className="w-full px-4 py-3 outline-none text-sm font-black" placeholder="Cost" /></div>
                     <div className="flex border-2 border-emerald-200 rounded-2xl overflow-hidden"><select name="domainChargeCurrency" defaultValue={editItem?.domainChargeCurrency || 'INR'} className="bg-emerald-50 px-2 outline-none font-black text-[9px]"><option value="NPR">NPR</option><option value="INR">INR</option></select><input name="domainCharge" type="number" step="0.01" defaultValue={editItem?.domainCharge} className="w-full px-4 py-3 outline-none text-sm font-black" placeholder="Charge" /></div>
                  </div>
                </div>
              </div>
            )}
            {(formType === 'hosting' || formType === 'both') && (
              <div className="p-8 rounded-[2.5rem] border-2 border-orange-200 bg-white space-y-6 shadow-sm">
                <p className="text-[11px] font-black text-orange-900 uppercase tracking-widest flex items-center gap-2"><Server size={18}/> Hosting Matrix</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="md:col-span-2"><input name="hostingPlan" defaultValue={editItem?.hostingPlan} className="w-full px-5 py-3.5 border-2 border-slate-300 rounded-2xl focus:border-orange-600 outline-none font-black text-orange-900" placeholder="Managed Hosting" /></div>
            ...
            <textarea name="notes" defaultValue={editItem?.notes} className="w-full px-8 py-6 border-2 border-slate-300 rounded-[2.5rem] bg-white text-slate-900 outline-none font-medium text-sm" rows={4} placeholder="Admin notes..." />
            <div className="flex gap-3">
              <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 bg-slate-200 text-slate-700 py-6 rounded-[2.5rem] font-black text-xl hover:bg-slate-300">DISMISS</button>
              <button type="submit" className="flex-[2] bg-slate-900 text-white py-6 rounded-[2.5rem] font-black text-xl shadow-2xl hover:bg-black transition-all">DEPLOY</button>
            </div>
         </form>
      </Modal>
    </div>
